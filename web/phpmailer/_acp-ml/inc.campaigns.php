...
if ($_GET['proc'] == 'del') {
  ...
  while ( $row = mysql_fetch_assoc($result) ) {
    ...
    $searchStr = $phpml['url_root'] . '/_msgimgs';
    $replacStr = $phpml['path_root'] . '/_msgimgs';
    $message   = str_replace($searchStr,$replacStr,$message);
    if ($phpml['trackOpens']) {
  ...
  <div class="tpl_table">
    <table class="tpl_listing tpl_form" width="100%" cellpadding="0" cellspacing="0">
      <tr><form name="tarea" action="index.php?pg=campaigns" method="post">
        <?php
        if ($dbListID != '') {
      ...
      if ( $phpml['editor'] == 'spaw2' ) {
        ...
        function insertToken(myField, myValue) {
...
if ( $phpml['editor'] == 'none' ) {
$pgArr['help']     = "<a href=\"\" onclick=\"insertToken(document.tarea.frmEditor, '{applicationname}');return false;\">{applicationname}</a><br />
  <a href=\"\" onclick=\"insertToken(document.tarea.frmEditor, '{email}');return false;\">{email}</a><br />
  <a href=\"\" onclick=\"insertToken(document.tarea.frmEditor, '{ip}');return false;\">{ip}</a><br />
  <a href=\"\" onclick=\"insertToken(document.tarea.frmEditor, '{firstname}');return false;\">{firstname}</a><br />
  <a href=\"\" onclick=\"insertToken(document.tarea.frmEditor, '{lastname}');return false;\">{lastname}</a><br />
  <a href=\"\" onclick=\"insertToken(document.tarea.frmEditor, '{mailinglist}');return false;\">{mailinglist}</a><br />
  <a href=\"\" onclick=\"insertToken(document.tarea.frmEditor, '{msgid}');return false;\">{msgid}</a><br />
  <a href=\"\" onclick=\"insertToken(document.tarea.frmEditor, '{onlineviewurl}');return false;\">{onlineviewurl}</a><br />
  <a href=\"\" onclick=\"insertToken(document.tarea.frmEditor, '{reg_date}');return false;\">{reg_date}</a><br />
  <a href=\"\" onclick=\"insertToken(document.tarea.frmEditor, '{remote_host}');return false;\">{remote_host}</a><br />
  <a href=\"\" onclick=\"insertToken(document.tarea.frmEditor, '{unsubscribe}');return false;\">{unsubscribe}</a><br />
  are variables that will get substituted with the subscriber&#039;s data at time of sending. Put your cursor at insertion
  point in the message, and click on the variable with your mouse.
...
$pgArr['help']     = "{applicationname}<br />
  ...
  Put your cursor at insertion point in the message, and use the drop down in the WYSIWYG editor.<br />";
}
if ( $_GET['proc'] == 'edit' ) {
$pgArr['help']    .= "<hr />
