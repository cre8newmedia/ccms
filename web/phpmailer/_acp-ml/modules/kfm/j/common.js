...
if(typeof(str)!='string')alert(typeof(str)+"\n"+str);if(!context)context='kfm';if(!vars)vars=[];if(!window.lang)window.lang=[];if(!window.lang[kfm_vars.lang])window.lang[kfm_vars.lang]=[];if(htmlonly){if(window.lang[kfm_vars.lang][str]){el='<span class="kfmlang kfmlang_'+str.toLowerCase().replace(/[^a-z0-9]/g,'')+'">'+window.lang[kfm_vars.lang][str].text+'</span>';window.lang[kfm_vars.lang][str].requested=1;}
else el='<span class="kfmlang kfmlang_'+str.toLowerCase().replace(/[^a-z0-9]/g,'')+'">...</span>';}
else{el=document.createElement('span');el.className='kfmlang kfmlang_'+str.toLowerCase().replace(/[^a-z0-9]/g,'');el.original=[str,context,vars];if(window.lang[kfm_vars.lang][str]){el.appendChild(document.createTextNode(window.lang[kfm_vars.lang][str].text));window.lang[kfm_vars.lang][str].requested=1;}
else el.appendChild(document.createTextNode('...'));}
if(!window.lang[kfm_vars.lang][str]){x_kfm_translate(str,context,kfm_vars.lang,function(res){if(window.lang[kfm_vars.lang][str])return;window.lang[kfm_vars.lang][str]={'text':res,'requested':1}
setTimeout(function(){$j('.kfmlang_'+str.toLowerCase().replace(/[^a-z0-9]/g,'')).text(window.lang[kfm_vars.lang][str].text);},1);});}
return el;}
...
function kfm_kaejax_sendRequests(uri){var t=window.kfm_kaejax_timeouts[uri],callbacks=window.kfm_kaejax_timeouts[uri].callbacks;t.callbacks=null;window.kfm_kaejax_timeouts[uri]=null;var x=new XMLHttpRequest(),post_data="kaejax="+escape(Json.toString(t)).replace(kfm_regexps.plus,'%2B').replace(kfm_regexps.ascii_stuff,'%u00$1').replace(/\n/g,' ');post_data=kfm_sanitise_ajax(post_data);x.open('POST',uri,true);x.setRequestHeader("Method","POST "+uri+" HTTP/1.1");x.setRequestHeader("Content-Type","application/x-www-form-urlencoded");x.onreadystatechange=function(){if(x.readyState!=4){return;}
var r=x.responseText;if(r.substring(0,5)=='error'){return kfm.alert(r);}
var v=eval('('+unescape(r)+')');var f,p,i;if(v.errors.length)kfm.showErrors(v.errors);if(v.messages.length)kfm.showMessages(v.messages);for(i=0;i<t.c.length;++i){f=callbacks[i];p=[];if($type(f)=='array'){p=f;f=f[0];}
f(v.results[i],p);}};x.send(post_data);}
